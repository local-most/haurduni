    ...
    <div class="col-12">
        <div class="card card-outline card-blue">
            <div class="card-header">
                <h3 class="card-title" id="card-title">Edit Produk</h3>
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="row">
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        ...
                                        <input type="file" name="foto1" class="form-control" accept="image/*" onchange="loadImage(event, 'preview-img-1')">
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <select class="form-control" name="kategori_id" required>
                                    @foreach($kategori as $row)
                                    	<option @if($produk->kategori_id == $row->id) selected @endif value="{{ $row->id }}">{{ $row->nama }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <select class="form-control" name="satuan_id" required>
                                    @foreach($satuan as $row)
                                        <option @if($produk->satuan_id == $row->id) selected @endif value="{{ $row->id }}">{{ $row->nama }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <input name="harga" id="harga" class="form-control" placeholder="Harga Produk" required id="rupiah" oninput="generateDiskonRupiah()" value="{{ $produk->harga }}">
                            </div>
                            <div class="form-group">
                                <label>Stok Produk<strong class="text-danger">*</strong></label>
                                <input type="text" name="stok" class="form-control" placeholder="Stok Produk" required onkeypress="return event.charCode >= 48 && event.charCode <=57" value="{{ $produk->stok }}">
                            </div>
                            <div class="form-group">
                                <label>Berat Produk<strong class="text-danger">*</strong></label>
                                <input type="text" name="berat" class="form-control" placeholder="Berat Produk" required onkeypress="return event.charCode >= 48 && event.charCode <=57" value="{{ $produk->berat }}">
                            </div>
                            <div class="form-group">
                                @php
                                    $diskon_rupiah = count($produk->Diskon) > 0 ? $produk->Diskon[0]->harga : 0;

                                    $harga = $produk->harga;
                                    $hasil = $diskon_rupiah/$harga*100;
                                @endphp
                                <label>Diskon %<strong class="text-danger"></strong></label>
                                <input type="text" id="diskon_persen" value="{{ $hasil }}" name="diskon_persen" oninput="generateDiskonRupiah()" class="form-control" placeholder="Diskon %" required onkeypress="return event.charCode >= 48 && event.charCode <=57">
                            </div>
                            <div class="form-group">
                                <label>Diskon Rupiah<strong class="text-danger"></strong></label>
                                <input type="text" id="diskon_rupiah" value="{{ count($produk->Diskon) > 0 ? $produk->Diskon[0]->harga : null }}" name="diskon_rupiah" class="form-control" placeholder="Rp. 0" required onkeypress="return event.charCode >= 48 && event.charCode <=57" readonly>
                            </div>
                            <!-- <div class="form-group">
                                ...
                                <select id="selectwarna" class="form-control select2" name="warna_id[]" multiple="multiple" data-placeholder="Pilih Warna">
                                    ...
                                    @foreach($warna as $row)
                                        <option {{in_array($row->id, explode(',',$produk->warna_id)) ?'selected':''}} value="{{ $row->id }}">{{ $row->nama }}</option>
                                    @endforeach
                                </select>
                            </div> -->
                            <div class="form-group">
                                <label>Deskripsi Produk<strong class="text-danger">*</strong></label>
                                <textarea id="content" name="deskripsi">{{ $produk->deskripsi }}</textarea>
                            </div>
