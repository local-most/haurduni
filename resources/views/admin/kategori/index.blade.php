		...
		<div class="card">
			<div class="card-header">
				<div class="float-right">
					<a href="{{ route('admin.kategori.create') }}" class="btn btn-primary btn-sm">Tambah Data</a>
				</div>
			</div>
			<div class="card-body">
				@if( session('msg') )
						...
						<tbody>
							...
							<tr>
								...
								<td class="text-center" width="150px">
									<a href="{{ route('admin.kategori.edit', $row->id) }}" class="btn btn-success btn-sm mt-1 mb-1"><i class="fas fa-edit"></i></a>
									<button type="button" class="btn btn-sm btn-danger btn-delete mt-1 mb-1" data-id="{{ $row->id }}"><i class="fas fa-trash"></i></button>
								</td>
								<td class="text-center">{{ $row->nama }} </td>
								<td class="text-center">{{ $row->is_color == true ? 'Aktif' : 'Nonaktif' }} </td>
							</tr>
							@endforeach
						</tbody>
					</table>
