                            ...
                            <div class="">
                                        ...
                                        <div id="alamat" style="display: none;">
                                            <div class="float-left">
                                                <p>
                                                    ...
                                                    <small>
                                                        <b>{{ auth()->user()->nama }}</b>
                                                        {{ auth()->user()->nohp ? '('.auth()->user()->nohp.')' : '(Harap isi nomor HP)' }} <br>
                                                        <tag style="color:gray;">
                                                            Wilayah {{ auth()->user()->wilayah_id ? auth()->user()->Wilayah->nama : '' }}, {{ auth()->user()->alamat }}
                                                        </tag>
                                                    </small>
                                                </p>
                                            </div>
                                            <div class="float-right">
                                                <a href="{{ route('profile') }}" class="btn"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                                            </div>
                                        </div>
                                        <div id="ambil-ke-toko" style="display: none;">
                                            <div class="alert btn-info">
                                                Barang yang kamu pesan, silahkan ambil ke Toko langsung.
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <table class="table">
                                    <thead style="display: none;">
                                        <tr>
                                            <td width="15%" class="text-center" style="display: none;">
                                    ...
                                    <tbody>
                                        ...
                                        <tr style="display: {{ $row->status == 1 ? 'show' : 'none' }}" id="display-ada-{{ $row->id }}">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{ route('pembelian.store') }}" method="POST">
                            @csrf
                            <input type="hidden" name="keranjang_id[]" id="keranjang-id">
                            ...
                            <div class="form-group d-block">
                                ...
                                <div class="float-right"><strong id="total" style="color: orange;">Rp0</strong></div>
                            </div>
                            <br>
                            <div class="form-group mt-3">
                                <button type="submit" class="form-control btn btn-sm" style="background-color: #3b4a6b; color: white;" id="total_beli">Bayar Sekarang</button>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    function getKeranjang() {
        ...
        function(res){
            ...
            if (res.jumlah_barang >= 1) {
                document.getElementById('total_beli').disabled = false;
            }else{
                document.getElementById('total_beli').disabled = true;
            }
            ...
            document.getElementById("total_barang").innerHTML = 'Total Tagihan';
            document.getElementById("total").innerHTML = res.total_bayar_view;

            document.getElementById("keranjang-id").value = res.keranjang_id;
            document.getElementById("store-sub-total-harga").value = res.total_harga;
            document.getElementById("store-total-harga").value = res.total_harga_;
            document.getElementById("store-ongkir").value = res.total_ongkir;
